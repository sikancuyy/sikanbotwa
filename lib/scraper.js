...
let btchIgdl = null;
let btchYoutube = null;
let btchYts = null;
try {
  const btch = require('btch-downloader');
  if (btch) {
    if (typeof btch.igdl === 'function') btchIgdl = btch.igdl;
    if (typeof btch.youtube === 'function') btchYoutube = btch.youtube;
    if (typeof btch.yts === 'function') btchYts = btch.yts;
  }
} catch (_) {}
...
function runYtDlp(args) {
  return new Promise((resolve, reject) => {
    ...
    if (!finalArgs.includes('--extractor-args')) {
      finalArgs.unshift('--extractor-args', 'youtube:player_client=android,ios,web');
    }
    ...
    const timeout = setTimeout(() => {
      ...
      reject(new Error('Proses yt-dlp timed out'));
    }, 60000);
...
async function searchYouTube(query, limit = 5) {
  // 1. Coba via API btch.yts jika tersedia
  if (btchYts) {
    try {
      const res = await btchYts(query);
      const vList = res?.result?.videos || res?.result?.all?.filter((item) => item.type === 'video') || [];
      if (vList.length > 0) {
        return vList.slice(0, limit).map((v) => ({
          id: v.videoId || (v.url ? v.url.split('v=')[1] : ''),
          title: v.title || 'No Title',
          duration: v.timestamp || (v.seconds ? `${v.seconds}s` : 'Unknown'),
          author: v.author?.name || 'Channel',
          views: typeof v.views === 'number' ? `${v.views.toLocaleString('id-ID')} views` : (v.views || '0 views'),
  ...
  // 2. Fallback ke scraping mobile YouTube
  try {
    ...
    if (match) {
      ...
      const items = sections?.[0]?.itemSectionRenderer?.contents || sections || [];
      const results = [];
      for (const it of items) {
        const v = it.videoRenderer || it.compactVideoRenderer;
        if (v && v.videoId) {
          results.push({
            id: v.videoId,
            title: v.title?.runs?.[0]?.text || v.title?.simpleText || 'No Title',
            duration: v.lengthText?.simpleText || v.lengthText?.runs?.[0]?.text || 'Unknown',
            author: v.ownerText?.runs?.[0]?.text || v.shortBylineText?.runs?.[0]?.text || 'Channel',
  ...
  // 3. Fallback ke yt-dlp jika web scraping gagal
  try {
...
/**
 * Download YouTube Audio (.play / .ytm)
 */
async function downloadYouTubeAudio(queryOrUrl, outputFile) {
  ...
  // 1. Coba unduh via API btchYoutube jika target adalah link YouTube
  if (btchYoutube && /youtu\.?be/i.test(target)) {
    try {
      const ytData = await btchYoutube(target);
      if (ytData && ytData.mp3) {
        const res = await axios.get(ytData.mp3, {
  ...
  // 2. Fallback ke yt-dlp lokal dengan android/ios extractor args
  await runYtDlp([
    target,
...
async function downloadYouTubeVideo(queryOrUrl, outputFile) {
  ...
  if (btchYoutube && /youtu\.?be/i.test(target)) {
    try {
      const ytData = await btchYoutube(target);
      if (ytData && ytData.mp4) {
        const res = await axios.get(ytData.mp4, {
          responseType: 'arraybuffer',
          timeout: 60000,
          headers: {
            'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36'
  ...
  // 2. Fallback ke yt-dlp lokal dengan format fleksibel
  await runYtDlp([
    target,
    '--extractor-args', 'youtube:player_client=android,ios,web',
