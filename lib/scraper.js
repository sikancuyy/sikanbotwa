...
function runYtDlp(args) {
  return new Promise((resolve, reject) => {
    ...
    const ytdlpBin = (isWindows && fs.existsSync(path.join(__dirname, '..', 'yt-dlp.exe')))
    ...
    if (ffmpegBin && !finalArgs.includes('--ffmpeg-location')) {
      finalArgs.push('--ffmpeg-location', ffmpegBin);
    }
...
async function getTikTok(url) {
  ...
  const parseTikWMData = (d) => {
    ...
    return {
      ...
      videoUrl: d.play || d.wmplay || d.hdplay || null,
      images,
  ...
  try {
    ...
    if (res.data && res.data.code === 0 && res.data.data) {
      return parseTikWMData(res.data.data);
    }
  } catch (_) {}
  ...
  try {
    ...
    if (resPost.data && resPost.data.code === 0 && resPost.data.data) {
      return parseTikWMData(resPost.data.data);
    }
  } catch (_) {}
...
async function getInstagram(url) {
  ...
  // Strategi 1: btch-downloader igdl
  if (btchIgdl) {
    try {
      const res = await btchIgdl(targetUrl);
  ...
  // Strategi 2: Backend Tioo igdl langsung
  try {
    const res = await axios.get(`https://backend1.tioo.eu.org/igdl?url=${encodeURIComponent(targetUrl)}`, {
      timeout: 20000,
      headers: {
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64)'
      }
  ...
  // Strategi 3: Bochil instagramdl
  if (bochilIg && typeof bochilIg.instagramdl === 'function') {
    try {
      const bRes = await bochilIg.instagramdl(targetUrl);
  ...
  throw new Error('Gagal mengambil konten Instagram. Pastikan link publik dan valid.');
}
  ...
  try {
    ...
    if (bochilIg && typeof bochilIg.instagramStory === 'function') {
      try {
        ...
        const bStory = await bochilIg.instagramStory(cleanUser);
